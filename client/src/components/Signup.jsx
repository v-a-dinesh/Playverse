...
import { googleSignIn, signUp, generateOtp } from "../api/index";
import OTP from "./OTP";
...
const SignUp = ({ setSignUpOpen, setSignInOpen }) => {
  ...
  const [initialOtpSent, setInitialOtpSent] = useState(false); // Add this state
  ...
  const handleSignUp = async (e) => {
    ...
    if (!disabled && !initialOtpSent) {
      setLoading(true);
      try {
        // Send OTP here
        const res = await generateOtp(email, name);
        if (res.status === 200) {
          dispatch(
            openSnackbar({
    ...
    } else if (initialOtpSent) {
      // If OTP was already sent, just show the OTP component
      setOtpSent(true);
    }
  };
        ...
        <Wrapper>
          ...
          {!otpSent ? (
            <>
              ...
              >
                {Loading ? (
                  ...
                  <>
                    <GoogleIcon src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c1/Google_%22G%22_logo.svg/1200px-Google_%22G%22_logo.svg.png" />
                    Sign In with Google
                  </>
                )}
              </OutlinedBox>
            ...
            <OTP
              ...
              initialOtpSent={initialOtpSent} // Pass this prop
            />
          )}
          <LoginText>
